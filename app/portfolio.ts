...
const additions = newPhotos.map((photo) => photo.src);
export const visibleImages: string[] = [...additions, ...images].filter((src: string) => !hidden.has(src));
const keep = (src: string) => !hidden.has(src);
export const heroImage = additions.find(keep) ?? visibleImages[0];
...
export const featuredImages = [...new Set([...additions.filter(keep), ...originalFeatured.filter(keep), ...visibleImages])].slice(0, 6);
// Filter the original gallery selection without changing the remaining photographs.
export const galleryImages = [...additions, ...images.filter((_: string, i: number) => i % 7 === 0).slice(0, 30)].filter(keep);
